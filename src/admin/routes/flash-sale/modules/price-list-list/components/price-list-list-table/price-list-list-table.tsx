import { Button, Container, Heading, Text } from '@medusajs/ui';
import { keepPreviousData } from '@tanstack/react-query';
import { useTranslation } from 'react-i18next';
import { Link } from 'react-router-dom';
import { DataTable } from '../../../../../../components/table/data-table';
import { useFlashSales } from '../../../../../../hooks/api/flash-sales';
import { useDataTable } from '../../../../../../hooks/use-data-table';
import { usePricingTableColumns } from './use-pricing-table-columns';
import { usePricingTableFilters } from './use-pricing-table-filters';
import { useFlashSaleTableQuery } from './use-pricing-table-query';

const PAGE_SIZE = 20;

export const PriceListListTable = () => {
	const { t } = useTranslation();

	const { searchParams, raw } = useFlashSaleTableQuery({
		pageSize: PAGE_SIZE,
	});

	const { price_lists, isPending, count, isError, error } = useFlashSales(
		searchParams,
		{
			placeholderData: keepPreviousData,
		},
	);

	const filters = usePricingTableFilters();
	const columns = usePricingTableColumns();

	const { table } = useDataTable({
		data: price_lists || [],
		columns,
		count,
		enablePagination: true,
		getRowId: (row) => row.id,
		pageSize: PAGE_SIZE,
	});

	if (isError) {
		throw error;
	}

	return (
		<Container className='divide-y p-0'>
			<div className='flex items-center justify-between px-6 py-4'>
				<div>
					<Heading>Flash Sale</Heading>
					<Text className='text-ui-fg-subtle' size='small'>
						Create flash sale prices for specific conditions.
					</Text>
				</div>
				<Button size='small' variant='secondary' asChild>
					<Link to='create'>{t('actions.create')}</Link>
				</Button>
			</div>
			<DataTable
				table={table}
				columns={columns}
				count={count}
				filters={filters}
				orderBy={[
					{ key: 'title', label: t('fields.title') },
					{ key: 'status', label: t('fields.status') },
					{ key: 'created_at', label: t('fields.createdAt') },
					{ key: 'updated_at', label: t('fields.updatedAt') },
				]}
				queryObject={raw}
				pageSize={PAGE_SIZE}
				navigateTo={(row) => row.original.id}
				isLoading={isPending}
				pagination
				search
			/>
		</Container>
	);
};
