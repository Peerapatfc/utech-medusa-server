...
function DetailsTab({ form, product }: DetailsTabProps) {
	...
	useEffect(() => {
		setTimeout(() => {
			form.setValue('manage_inventory', true, {
				shouldDirty: true,
				shouldTouch: true,
				shouldValidate: true,
			});
			// form.setValue("inventory_kit", false, {
			// 	shouldDirty: true,
			// 	shouldTouch: true,
			// 	shouldValidate: true,
			// });
	...
	useEffect(() => {
		if (options) {
			const variantTitle = Object.values(options).filter(Boolean).join('-');
			if (variantTitle) {
				const titleWithProductName = `${product.title} - ${variantTitle}`;
				form.setValue('title', titleWithProductName);
			}
		}
	}, [options, form, product.title]);
