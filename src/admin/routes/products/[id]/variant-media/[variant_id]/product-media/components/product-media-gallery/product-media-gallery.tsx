...
export const ProductMediaGallery = ({ product }: ProductMediaGalleryProps) => {
	...
	const handleDeleteCurrent = async () => {
		...
		mediaToKeep.map(async (url) => {
			await formDataImages.append('images', url);
		});
		await fetch(
			`/admin/custom/products/${id}/variants/${variant_id}/images`,
			{
				method: 'POST',
				body: formDataImages,
			},
		...
		toast.success(t('general.success'), {
			description: 'Variant images was successfully deleted.',
		});
	};
