...
import type { HttpTypes, OrderStatus } from '@medusajs/types';
import {
	OrderNoCell,
	OrderNoHeader,
} from '../../../components/table/table-cells/order/order-no-cell/order-no-cell';
import {
	OrderStatusCell,
	OrderStatusHeader,
} from '../../../components/table/table-cells/order/order-status-cell/order-status-cell';

interface IAdminOrder extends HttpTypes.AdminOrder {
	status: OrderStatus;
}

// We have to use any here, as the type of Order is so complex that it lags the TS server
const columnHelper = createColumnHelper<IAdminOrder>();
...
export const useOrderTableColumns = (props: UseOrderTableColumnsProps) => {
	...
	const columns = useMemo(
		() => [
			...
			// 	},
			// }),
			columnHelper.accessor((row) => row.metadata?.order_no, {
				id: 'metadata.order_no',
				header: () => <OrderNoHeader />,
				cell: ({ row }) => {
					const orderNo = row.original.metadata?.order_no as string;
					return <OrderNoCell orderNo={orderNo} />;
				},
			}),
