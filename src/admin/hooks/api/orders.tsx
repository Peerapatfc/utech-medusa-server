...
const _orderKeys = queryKeysFactory(ORDERS_QUERY_KEY) as TQueryKey<'orders'> & {
	preview: (orderId: string, dynamicKey?: string | number) => any;
	changes: (orderId: string, dynamicKey?: string | number) => any;
	lineItems: (orderId: string, dynamicKey?: string | number) => any;
};
...
export const useOrderTaxInvoiceAddress = (id: string) => {
	const { data } = useQuery({
		queryFn: async () =>
			sdk.client.fetch(`/admin/custom/orders/${id}/tax-invoice-address`),
		queryKey: ['order', id, 'tax-invoice-address'],
	});

	return data as {
		tax_invoice_address: HttpTypes.AdminOrder['shipping_address'];
	};
};

export const useSaveOrderItemSerialNumber = (
	orderId: string,
	options?: UseMutationOptions<any, FetchError, any>,
) => {
	return useMutation({
		mutationFn: (payload) =>
			sdk.client.fetch(`/admin/custom/orders/${orderId}/serial-number`, {
				method: 'POST',
				body: payload,
			}),
		onSuccess: (data: any, variables: any, context: any) => {
