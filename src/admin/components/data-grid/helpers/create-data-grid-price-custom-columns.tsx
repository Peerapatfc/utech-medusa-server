...
import {
	...
	Inner,
} from '../components/data-grid-currency-cell';
...
import { DataGridCellContainer } from '../components/data-grid-cell-container';
...
export const createDataGridPriceCustomColumns = <
	TData,
	TFieldValues extends FieldValues,
>({
	currencies,
	...
	return [
		...
		columnHelper.column({
			...
			field: (context) => {
				...
				return getFieldName(context, '');
			},
			type: 'number',
			header: () => (
				<div className='flex w-full items-center justify-between gap-3'>
					<span className='truncate' title={'Flash Sale Qty'}>
						{'Flash Sale Qty'}
					</span>
				</div>
			),
