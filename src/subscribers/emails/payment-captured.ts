...
export default async function paymentCaptured({
	event,
	container,
	...
	logger.info(`Payment captured email triggered for payment: ${paymentId}`);
	...
	try {
		...
		orderId = data[0]?.order?.id;
	} catch (error) {
		logger.error(
			`Error payment captured email, error: ${error.message}`,
			error,
		);
		return;
	}
	...
	if (!orderId) {
		logger.error(
			`Error payment captured email, error: No order found for payment: ${paymentId}`,
		);
		return;
	}
	...
	try {
		...
		const capturedAt =
			orderDetail.payment_collections?.[0]?.payments?.[0]?.captured_at;
		...
		const payload = {
			...
			date: dayjs(capturedAt).tz('Asia/Bangkok').format('DD/MM/YYYY'),
			time: dayjs(capturedAt).tz('Asia/Bangkok').format('HH:mm'),
		...
		let templateId = process.env.SENDGRID_ORDER_CONFIRMED_ID;
		if (isPreOrder) {
			templateId = process.env.SENDGRID_PRE_ORDER_CONFIRMED_ID;
		}
		...
		logger.error(`Error sending order placed email: ${error?.message}`, error);
	}
}
...
export const config: SubscriberConfig = {
	...
	context: {
		subscriberId: 'payment-captured-email',
	},
};
